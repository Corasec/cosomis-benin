            ...
            file = compress_image(
                ...
                (
                    ...
                    if file.size > (1 * 1024 * 1024)
                        ...
                        else (file.size / (1024 * 1024))
                    )
                ),
            )

        if file and (
            ("subproject_step" in data and data["subproject_step"])
            or ("subproject_level" in data and data["subproject_level"])
        ):
            ...
            if "id" in data and data["id"]:
                subproject_file = SubprojectFile.objects.filter(id=data["id"]).first()
            ...
            subproject_file.date_taken = datetime.strptime(
                data["date_taken"], "%Y-%m-%d"
            ).date()
