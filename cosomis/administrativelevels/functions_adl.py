...
def get_cascade_administrative_levels_by_administrative_level_id(_id):
    ...
    if _id:
        ...
        if _type == "Region":
            ...
            prefectures = ads
            communes = AdministrativeLevel.objects.filter(
                parent_id__in=[o.id for o in prefectures]
            )
            cantons = AdministrativeLevel.objects.filter(
                parent_id__in=[o.id for o in communes]
            )
            villages = AdministrativeLevel.objects.filter(
                parent_id__in=[o.id for o in cantons]
            )
...
def get_cascade_villages_by_administrative_level_id(_ids):
    ...
    if _ids:
        ...
        ad_objects = AdministrativeLevel.objects.filter(
            id__in=[int(_id) for _id in _ids if _id]
        )
        ...
        for ad_obj in ad_objects:
            if ad_obj:
                ...
                for p in datas["prefectures"]:
                    [
                        datas["communes"].append(o)
                        for o in p.administrativelevel_set.get_queryset()
                    ]

                for c in datas["communes"]:
                    [
                        datas["cantons"].append(o)
                        for o in c.administrativelevel_set.get_queryset()
                    ]

                for c in datas["cantons"]:
                    [
                        datas["villages"].append(o)
                        for o in c.administrativelevel_set.get_queryset()
                    ]
        ...
        for ad_obj in ad_objects:
            if ad_obj:
                ...
                datas = {
                    ...
                    "cantons": ads if _type == "Commune" else [],
                    "villages": (
                        [o.id for o in ads] if _type in ("Canton", "Village") else []
                    ),
                }
                for p in datas["prefectures"]:
                    [
                ...
                for c in datas["cantons"]:
                    [
                        datas["villages"].append(int(o.id))
                        for o in c.administrativelevel_set.get_queryset()
                    ]
