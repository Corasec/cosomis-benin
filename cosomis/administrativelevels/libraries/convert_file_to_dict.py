...
def conversion_file_csv_merger_to_dict(
    file_csv, sheet="Sheet1", method="ffill", columns_fillna=[], axis=0
) -> dict:
    read_file = pd.read_csv(file_csv, sheet)
    if columns_fillna:
        read_file[columns_fillna] = read_file[columns_fillna].fillna(
...
def conversion_file_xlsx_merger_to_dict(
    file_xlsx, sheet="Sheet1", method="ffill", columns_fillna=[], axis=0
) -> dict:
    read_file = pd.read_excel(file_xlsx, sheet)
    if columns_fillna:
        read_file[columns_fillna] = read_file[columns_fillna].fillna(
