...
class VillageDetailView(PageMixin, LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            context.get("object") and context.get("object").type == "Village"
        ):  # Verify if the administrativeLevel type is Village
            return context
        raise Http404
...
class AdministrativeLevelCreateView(
    ...
    def get_context_data(self, **kwargs):
        ...
        context["form"] = AdministrativeLevelForm(
            self.get_parent(self.request.GET.get("type"))
        )
        return context
...
class AdministrativeLevelUpdateView(
    ...
    def get_context_data(self, **kwargs):
        ...
        context["form"] = AdministrativeLevelForm(
            self.get_parent(self.request.GET.get("type")), instance=self.get_object()
        )
        return context
...
class UploadCSVView(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if _type in ("priority", "subproject"):
            ...
            try:
                datas = convert_file_to_dict.conversion_file_xlsx_to_dict(
                    request.FILES.get("file"), request.POST.get("sheet_name")
                )
            except pd.errors.ParserError as exc:
                datas = convert_file_to_dict.conversion_file_csv_to_dict(
                    request.FILES.get("file"), request.POST.get("sheet_name")
                )
            except Exception as exc:
                messages.info(request, _("An error has occurred..."))
...
class ObstaclesListView(PageMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.setdefault(
            "administrativelevel_village", ObstaclesListView.administrativelevel_village
        )
        ctx.setdefault("OBSTACLES_FOCUS_GROUP", OBSTACLES_FOCUS_GROUP)

        ctx.setdefault(
            "obstacles",
            VillageObstacle.objects.filter(
                administrative_level_id=ObstaclesListView.administrative_level_id
            ).order_by("ranking"),
        )
    ...
    def get(self, request, *args, **kwargs):
        try:
            ObstaclesListView.administrative_level_id = kwargs[
                "administrative_level_id"
            ]
            ObstaclesListView.administrativelevel_village = (
...
def obstacle_delete(request, obstacle_id):
    ...
    return redirect(
        "administrativelevels:priorities_obstacles",
...
class GoalsListView(PageMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.setdefault(
            "administrativelevel_village", GoalsListView.administrativelevel_village
        )
        ctx.setdefault("GOALS_FOCUS_GROUP", GOALS_FOCUS_GROUP)

        ctx.setdefault(
            "goals",
            VillageGoal.objects.filter(
                administrative_level_id=GoalsListView.administrative_level_id
            ).order_by("ranking"),
        )
...
def goal_delete(request, goal_id):
    ...
    return redirect(
        "administrativelevels:priorities_goals",
        administrative_level_id=administrative_level_id,
    )
...
class PrioritiesListView(PageMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.setdefault(
            "administrativelevel_village",
            PrioritiesListView.administrativelevel_village,
        )
        ctx.setdefault("components", Component.objects.all())
        ctx.setdefault(
            "goals",
            VillageGoal.objects.filter(
                administrative_level=PrioritiesListView.administrativelevel_village
            ),
        )

        ctx.setdefault(
            "priorities",
            VillagePriority.objects.filter(
                administrative_level_id=PrioritiesListView.administrative_level_id
            ).order_by("ranking"),
        )
    ...
    def get(self, request, *args, **kwargs):
        try:
            PrioritiesListView.administrative_level_id = kwargs[
                "administrative_level_id"
            ]
            PrioritiesListView.administrativelevel_village = (
                AdministrativeLevel.objects.get(
    ...
    def post(self, request, *args, **kwargs):
        ...
        climate_changing_contribution = request.POST.get(
            "climate_changing_contribution"
        )
            ...
            for key in request.POST:
                if key and type(key) is str and "-" in key and key[-1].isdigit():
                    try:
                        ...
                        climate_changing_contribution = request.POST.get(
                            "climate_changing_contribution-" + id_str
                        )
                        if component_id and climate_changing_contribution:
                            priority = VillagePriority.objects.get(id=id)
                            priority.component_id = component_id
                            priority.proposed_men = (
                                int(proposed_men) if proposed_men else 0
                            )
                            priority.proposed_women = (
                                int(proposed_women) if proposed_women else 0
                            )
...
def priority_delete(request, priority_id):
    ...
    return redirect(
        "administrativelevels:priorities_priorities",
        administrative_level_id=administrative_level_id,
    )
...
class GeographicalUnitUpdateView(
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["form"] = GeographicalUnitForm(
            initial={
                "villages": [
                    cat
                    for cat in self.get_object()
                    .get_villages()
...
class CVDListView(PageMixin, LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search:
            ...
            search = search.upper()
            return Paginator(CVD.objects.filter(name__icontains=search), 100).get_page(
                page_number
            )
        else:
