...
from administrativelevels.serializers import (
    ...
    CVDWithAdministrativeLevelSerializer,
)
...
class RestGetAdministrativeLevelByUser(APIView):
    ...
    def post(self, request, type_adl: str, project_id: int, *args, **kwargs):
            ...
            if type_adl.title() in ("Village", "Canton"):
                if parent_id:
                    administrative_levels = AdministrativeLevel.objects.filter(
                        type=type_adl.title(), parent_id=parent_id
                    )
                else:
                    administrative_levels = AdministrativeLevel.objects.filter(
                        type=type_adl.title()
        ...
        serializer = AdministrativeLevelSerializer(
            paginated_data, many=True, initial={"user": user}
        )
...
class RestGetACVDByUser(APIView):
    ...
    def post(self, request, project_id: int, *args, **kwargs):
        ...
        if not hasattr(user, "no_sql_user"):
            if parent_id:
                administrative_levels = AdministrativeLevel.objects.filter(
                    type="Village", parent_id=parent_id
                )
            else:
                administrative_levels = AdministrativeLevel.objects.filter(
                    type="Village"
        ...
        cvds = CVD.objects.filter(
            pk__in=[adl.cvd.id for adl in administrative_levels if adl.cvd]
        )

        paginator = CustomPagination()
        paginated_data = paginator.paginate_queryset(cvds, request)
        serializer = CVDWithAdministrativeLevelSerializer(
            paginated_data, many=True, initial={"user": user}
        )
