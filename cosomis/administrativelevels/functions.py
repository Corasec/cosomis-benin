...
def get_administratives_levels_under_file_excel_or_csv(
    file_type="excel", params={"type": "All", "value_of_type": ""}
) -> str:
    ...
    datas = {
        ...
        "Village/localité": {},
        "Id Village/localité": {},
        "Village frontalier (1=oui, 0= non)": {},
        "Localité (Rural=1, urbain=0)": {},
        "Latitude": {},
        "Longitude": {},
    }
            ...
            for prefecture in AdministrativeLevel.objects.filter(parent=region):
                for commune in AdministrativeLevel.objects.filter(parent=prefecture):
                    for canton in AdministrativeLevel.objects.filter(parent=commune):
                        [
                            administratives_levels.append(village)
                            for village in canton.administrativelevel_set.get_queryset()
                        ]
...
def save_csv_datas_priorities_in_db(
    datas_file: dict,
    administrative_level_id=0,
    type_object="priority",
    cvd_ids=[],
    canton_ids=[],
) -> str:
        ...
        while count < long:
            for column in columns:
                try:
                    ...
                    estimated_cost = (
                        estimated_cost if not pd.isna(estimated_cost) else None
                    )
                    ...
                    for village in __village.split("/"):
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        text_errors += (
                                            ...
                                            if text_errors
                                            else f"Line N°{count}: {exc.__str__()}"
                                        )
                        ...
                        if not _is_object_error:
                            if administrative_level_id and int(
                                administrative_level.id
                            ) != int(administrative_level_id):
                                continue  # Continue without save the object if the village is specific and the village current is different

                            estimated_cost = (
                                float(estimated_cost) if estimated_cost else 0.0
                            )
                            ...
                            for char in _list_chars:
                                if char in list(_components.keys()):
                                    ...
                                    if _list_data[0].isdigit():
                                        ...
                                        name_priority = (
                                            str(data)[(len(_list_data[0]) + 1) :]
                                        ).strip()
                                    else:
                                        name_priority = str(data).strip()
                                    try:
                                        component = Component.objects.get(
                                            name=_components[char].upper()
                                        )
                                    except Exception as exc:
                                        component = None
                            ...
                            if (
                                type_object == "priority"
                                and not VillagePriority.objects.filter(
                                    name=name_priority,
                                ...
                                type_object == "subproject"
                                and not Subproject.objects.filter(
                                    short_name=name_priority,
                                    administrative_level=administrative_level,
                                    component=component,
                                )
                            ):
                                if estimated_cost and estimated_cost != 0:
                                    villages = VillagePriority.objects.filter(
                                        administrative_level=administrative_level,
                                        component=component,
                                    ...
                                    subproject.administrative_level = (
                                        administrative_level
                                    )
                                    ...
                                    if len(villages) == 0:
                                        ...
                                        subproject.target_female_beneficiaries = (
                                            villages[0].proposed_women
                                        )
                                        subproject.target_male_beneficiaries = villages[
                                            0
                                        ].proposed_men
    ...
    file_path = (
        "logs/errors/upload_priorities_logs_errors_"
        + str(datetime.today().replace(microsecond=0))
        .replace("-", "")
        .replace(":", "")
        .replace(" ", "_")
    ...
    if type_object == "subproject":
        file_path = file_path.replace(
            "upload_priorities_logs_errors_", "upload_subprojects_logs_errors_"
        )
