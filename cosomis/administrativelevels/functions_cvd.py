...
def save_cvd_instead_of_csv_file_datas_in_db(datas_file: dict, project_id=1) -> str:
    ...
    datas = {
        ...
        "N° DE COMPTE": {},
        "RIB": {},
        "Nom du président du CVD": {},
        "Tél du Président du CVD": {},
        "Nom du trésorier du CVD": {},
        "Tél du trésorier du CVD": {},
        "Nom du secrétaire du CVD": {},
        "Tél du secrétaire du CVD": {},
    }
    ...
    if datas_file:
        ...
        while count < long:
            try:
                ...
                rib = (
                    (str(int(rib)) if int(rib) > 9 else f"0{int(rib)}") if rib else None
                )
                ...
                try:
                    president_name_of_the_cvd = get_value(
                        datas_file["Nom du président du CVD"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    president_phone_of_the_cvd = get_value(
                        datas_file["Tél du Président du CVD"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    treasurer_name_of_the_cvd = get_value(
                        datas_file["Nom du trésorier du CVD"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    treasurer_phone_of_the_cvd = get_value(
                        datas_file["Tél du trésorier du CVD"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    secretary_name_of_the_cvd = get_value(
                        datas_file["Nom du secrétaire du CVD"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    secretary_phone_of_the_cvd = get_value(
                        datas_file["Tél du secrétaire du CVD"][count]
                    )
                except Exception as exc:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    amount_in_dollars = get_value(
                        datas_file["Montant alloué en dollars"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    amount_after_signature_of_contracts = get_value(
                        datas_file["Montant alloué apres signature"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    amount_in_dollars_after_signature_of_contracts = get_value(
                        datas_file["Montant alloué apres signature en dollars"][count]
                    )
                except Exception as exc:
                    pass
                try:
                    ...
                    pass
                try:
                    amount_transferred_in_dollars = get_value(
                        datas_file["Montant viré en dollars"][count]
                    )
                except Exception as exc:
                    pass
                ...
                if cvd:
                    if cvd_name != None:
                        cvd.name = cvd_name
                    cvd.bank = Bank.objects.filter(
                        Q(name=bank) | Q(abbreviation=bank)
                    ).first()
                    if bank_code != None:
                    ...
                    try:
                        if amount != None:
                            allocation = AdministrativeLevelAllocation.objects.filter(
                                cvd_id=cvd.id, amount=amount
                            ).first()
                            if not allocation:
                            ...
                            if amount_after_signature_of_contracts:
                                allocation.amount_after_signature_of_contracts = (
                                    amount_after_signature_of_contracts
                                )
                            if amount_in_dollars_after_signature_of_contracts:
                                allocation.amount_in_dollars_after_signature_of_contracts = (
                    ...
                    try:
                        if amount_transferred:
                            bank_transfer = BankTransfer.objects.filter(
                                cvd_id=cvd.id, amount_transferred=amount_transferred
                            ).first()
                            ...
                            if amount_transferred_in_dollars:
                                bank_transfer.amount_transferred_in_dollars = (
...
def get_cvd_under_file_excel_or_csv(file_type, administrative_levels_ids) -> str:
    ...
    cvds = CVD.objects.filter(
        ...
        "headquarters_village__parent__name",
        "name",
    )
    ...
    for elt in cvds:
        ...
        try:
            datas["REGION"][
                count
            ] = elt.headquarters_village.parent.parent.parent.parent.name
        except Exception as exc:
        ...
        try:
            datas["PREFECTURE"][
                count
            ] = elt.headquarters_village.parent.parent.parent.name
        except Exception as exc:
        ...
        datas["VILLAGES"][count] = "; ".join(
            [village.name for village in elt.get_villages()]
        )
