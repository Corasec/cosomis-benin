    ...
    def to_representation(self, instance, *args, **kwargs):
        ...
        if self.user:
            ...
            if not hasattr(self.user, "no_sql_user"):
                ...
                subprojects = get_subprojects_by_facilitator_id_and_project_id(
                    self.user.id, 1
                )
            ...
            data["number_subprojects"] = subprojects.filter(
                ...
                | Q(
                    ...
                    location_subproject_realized__parent__id=instance.id,
                )
...
class CVDWithAdministrativeLevelSerializer(serializers.ModelSerializer):
    geographical_unit = GeographicalUnitSerializer(many=False)
    administrativelevels = AdministrativeLevelSerializer(
        source="administrativelevel_set", many=True
    )
