...
class AssignAdministrativeLevelToFacilitator(BaseModel):
    administrative_level = models.ForeignKey(
        AdministrativeLevel, on_delete=models.CASCADE
    )
    ...
    class Meta:
        constraints = [
            ...
            )  # constraint defining that an assignment is only necessary in a village at a given time
        ]
    ...
    # handle unsupported constraint by mysql
    # def clean(self):
    #     if self.activated:
    #         if AssignAdministrativeLevelToFacilitator.objects.filter(
    #             administrative_level=self.administrative_level,
    #             project=self.project,
    #             activated=True
    #         ).exists():
    #             raise ValidationError('An active assignment already exists for this administrative level and project.')
