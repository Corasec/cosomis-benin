...
def get_subprojects_by_facilitator_id_and_project_id(facilitator_id, project_id) -> _QS:
    ...
    for (
        adl
    ) in combine_administrativelevels_assigned_by_facilitator_stabilized_and_project_id(
...
def get_administrativelevels_by_facilitator_id_and_project_id(
    facilitator_id, project_id, type_adl="Village", parent_id=None
) -> _QS:
    assigns_to_facilitator = AssignAdministrativeLevelToFacilitator.objects.filter(
        facilitator_id=facilitator_id, project_id=project_id, activated=True
    )
    ...
    administrativelevels = [
        assign.administrative_level
        for assign in assigns_to_facilitator
        if assign.administrative_level
        and (
            not parent_id
            or (
                parent_id
                and assign.administrative_level.parent
                and assign.administrative_level.parent_id == parent_id
            )
        )
    ]
                ...
                set(
                    [
                        ...
                        and (
                            ...
                            or (
                                parent_id
                                and adl.parent.parent
                                and adl.parent.parent_id == parent_id
                            )
                        )
                    ]
                )
    ...
    return AdministrativeLevel.objects.filter(
        pk__in=[a.pk for a in administrativelevels]
    )


def get_stabilized_administrativelevels_of_facilitators_by_project_id(
    facilitator, project_id, type_adl="Village", parent_id=None
) -> _QS:
    ...
    facilitators_stabilized = eadls.get_query_result(
        {"type": "adl", "representative.email": facilitator.email}
    )[:]
    ...
    if facilitators_stabilized:
        ...
        administrativelevels = [
            adl
            for adl in _administrativelevels
            if (
                not parent_id
                or (parent_id and adl.parent and adl.parent_id == parent_id)
            )
        ]
...
def combine_administrativelevels_assigned_by_facilitator_stabilized_and_project_id(
    facilitator, project_id, type_adl="Village", parent_id=None
) -> _QS:
    administrative_levels_stabilized = (
        get_stabilized_administrativelevels_of_facilitators_by_project_id(
            facilitator, project_id, type_adl.title(), parent_id
        )
    )
    administrative_levels_assigned_for_cdd_process = (
        get_administrativelevels_by_facilitator_id_and_project_id(
            facilitator.id, project_id, type_adl.title(), parent_id
        )
    )

    return list(
        set(
            list(administrative_levels_stabilized)
            + list(administrative_levels_assigned_for_cdd_process)
        )
    )
