...
from subprojects.forms import (
    SubprojectAddStepForm,
    SubprojectAddLevelForm,
)  # , DeleteConfirmForm
...
class SubprojectFormMixin(SubprojectMixin, generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("subproject_step_update_id"):
            obj = SubprojectStep.objects.get(
                id=self.kwargs["subproject_step_update_id"]
            )
...
class SubprojectStepGraphTemplateView(
    SubprojectMixin, AJAXRequestMixin, LoginRequiredMixin, generic.TemplateView
):
    ...
    def get_oldest_latest_and_percent(self, objects):
        ...
        for step in objects:
            if (
                hasattr(step, "step")
                and hasattr(step.step, "has_levels")
                and step.step.has_levels
            ):
                continue
            ...
            if step.begin and (
                (not oldest) or (oldest and list(oldest.keys())[0] > step.begin)
            ):
                oldest = {step.begin: step.wording}
            if step.end and (
                (not oldest) or (oldest and list(oldest.keys())[0] > step.end)
            ):
                oldest = {step.end: step.wording}

            if step.begin and (
                (not latest) or (latest and list(latest.keys())[0] < step.begin)
            ):
                latest = {step.begin: step.wording}
            if step.end and (
                (not latest) or (latest and list(latest.keys())[0] < step.end)
            ):
    ...
    def get_context_data(self, **kwargs):
        ...
        oldest_step, latest_step, dates_wording_with_percents_step = (
            self.get_oldest_latest_and_percent(context["subproject_steps"])
        )
        oldest_level, latest_level, dates_wording_with_percents_level = (
            self.get_oldest_latest_and_percent(subproject_levels)
        )
        ...
        dates_wording_with_percents = sorted(
            dates_wording_with_percents,
            key=lambda obj: f"{obj.get('date')} {obj.get('percent')}",
        )
...
class SubprojectStepAddTemplateView(
    SubprojectMixin, AJAXRequestMixin, LoginRequiredMixin, generic.TemplateView
):
...
# Add
class SubprojectStepAddFormView(
    AJAXRequestMixin,
    ModalFormMixin,
    LoginRequiredMixin,
    InfraPermissionRequiredMixin,
        ...
        if form and form.is_valid():
            ...
            if obj:
                ...
                if current_subproject_step and current_subproject_step.ranking:
                    ...
                    if ranking:
                        if (
                            current_subproject_step.wording == "Identifié"
                            and data["step"].ranking == 3
                        ):
                            ranking = 2
                        elif (
                            current_subproject_step.wording == "En cours"
                            and data["step"].ranking == 10
                        ):
                            ranking = 9
                        elif (
                            current_subproject_step.wording == "En cours"
                            and data["step"].ranking == 11
                        ):
                            ranking = 10
            ...
            if (data.get("step") and ranking and data["step"].ranking != ranking) or (
                not ranking and data["step"].ranking != 1
            ):
    ...
    def form_valid(self, form):
        ...
        if not self._obj:
            if subproject_step.step.ranking < 8 and subproject_step.step.ranking != 2:
                ...
                self.subproject.current_status_of_the_site = (
                    subproject_step.step.wording
                )

            if subproject_step.step.ranking == 3:
                self.subproject.approval_date_cora = subproject_step.begin

            self.subproject.current_level_of_physical_realization_of_the_work = str(
                subproject_step.step.percent
                if subproject_step.step.percent
                else subproject_step.step.wording
            )
            self.subproject.save()

        images = subproject_step.subproject.get_all_images()
        for file in [
            self.request.FILES.get("level_image"),
            self.request.FILES.get("level_other_file"),
        ]:
            if file:
                ...
                if not media_storage.exists(
                    ...
                    image = SubprojectFile.objects.filter(
                        subproject_step_id=subproject_step.id,
...
class SubprojectLevelAddFormView(
    AJAXRequestMixin,
    ModalFormMixin,
    LoginRequiredMixin,
    InfraPermissionRequiredMixin,
    ...
    def form_valid(self, form):
        ...
        try:
            subproject_step = SubprojectStep.objects.get(
                id=self.kwargs["subproject_step_id"]
            )
        except:
            raise Http404
        ...
        if not self._obj:
            ...
            self.subproject.current_level_of_physical_realization_of_the_work = str(
                subproject_level.percent if subproject_level.percent else "0"
            )
            self.subproject.save()
        ...
        for file in [
            ...
            if file:
                ...
                if not media_storage.exists(
                    ...
                    image = SubprojectFile.objects.filter(
                        subproject_level_id=subproject_level.id,
                        file_type=file.content_type,
                    ).first()
                    if not image:
                        image = SubprojectFile()
...
class FilesView(
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("subproject_step_id"):
            context["object"] = SubprojectStep.objects.get(
                id=self.kwargs["subproject_step_id"]
            )
