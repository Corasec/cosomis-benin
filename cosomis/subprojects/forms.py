...
class SubprojectForm(forms.ModelForm):
    ...
    level_of_achievement_donation_certificate = forms.ChoiceField(
        label=_("Level of donation certificate"), required=True
    )
    current_status_of_the_site = forms.ChoiceField(
        label=_(
        ...
        for label, field in self.fields.items():
            ...
            if label == "canton":
                ...
                self.fields[label].help_text = _(
                    "Fill in this field only if the subproject concerns all the villages in the canton."
                )
            ...
            if label == "component":
                self.fields[label].queryset = Component.objects.filter(
                    parent__name="Composante 1"
                )
...
class SubprojectAddStepForm(forms.ModelForm):
    ...
    level_other_file = forms.FileField(
        label=_("Another file considered important at this level"), required=False
    )
...
class SubprojectFilterForm(forms.Form):
    ...
    def __init__(self, has_all=True, *args, **kwargs):
        ...
        query_result_works_type_of_subproject = sorted(
            init_list + list(WORKS_TYPE_OF_SUB_PROJECT)
        )
        query_result_subproject_step = init_list + list(SUB_PROJECT_STEP_STANDART)

        self.fields["subproject_sectors"].widget.choices = (
            query_result_subproject_sectors
        )
        self.fields["subproject_types"].widget.choices = query_result_subproject_types
        self.fields["works_type_of_subproject"].widget.choices = (
            query_result_works_type_of_subproject
        )
