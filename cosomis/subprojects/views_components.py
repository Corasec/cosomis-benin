...
class SubprojectCommonComponent(SubprojectMixin, LoginRequiredMixin, generic.ListView):
    ...
    def get_results(self):
        subprojects = Subproject.objects.filter(
            full_title_of_approved_subproject=self.subproject.full_title_of_approved_subproject
        ).get_actifs()
        return {
...
class SubprojectsByAdministrativeLevelMixin(AdministrativeLevelMixin):
    ...
    def dispatch(self, request, pk: int, *args, **kwargs):
        ...
        if self.administrative_level.type == "Canton":
            self.villages = (
                self.administrative_level.administrativelevel_set.get_queryset()
            )
...
class SubprojectsBySectorANdAdministrativeLevelComponent(
    SubprojectsByAdministrativeLevelMixin, LoginRequiredMixin, generic.ListView
):
    ...
    def get_results(self):
        ...
        for subp in self.subprojects:
            if not subprojects_by_sector.get(subp.subproject_sector):
                subprojects_by_sector[subp.subproject_sector] = {
                    "number": 1,
                    "percent": float("%.2f" % ((1 / length_subprojects) * 100)),
                }
            else:
                subprojects_by_sector[subp.subproject_sector]["number"] += 1
                subprojects_by_sector[subp.subproject_sector]["percent"] = float(
                    "%.2f"
                    % (
                        (
                            subprojects_by_sector[subp.subproject_sector]["number"]
                            / length_subprojects
                        )
                        * 100
                    )
...
class SubprojectsByAdministrativeLevelComponent(
    SubprojectsByAdministrativeLevelMixin, LoginRequiredMixin, generic.ListView
):
...
class SummarySubprojectsByAdministrativeLevelComponent(
    SubprojectsByAdministrativeLevelMixin, LoginRequiredMixin, generic.ListView
):
        ...
        for subp in self.subprojects:
            if not summary_subprojects.get(subp.full_title_of_approved_subproject):
                summary_subprojects[subp.full_title_of_approved_subproject] = {
                    ...
                    "pk": subp.pk,
                    "projects_ids": [o.id for o in subp.get_all_projects],
                ...
                summary_subprojects[subp.full_title_of_approved_subproject][
                    ...
                    set(
                        ...
                        + [o.id for o in subp.get_all_projects]
                    )
                )
                summary_subprojects[subp.full_title_of_approved_subproject][
                    "financiers_ids"
                    ...
                    set(
                        ...
                        + [o.id for o in subp.get_all_financiers]
                    )
                )
