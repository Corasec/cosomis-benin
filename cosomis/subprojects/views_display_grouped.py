...
class DashboardSubprojectsMixin:
    ...
    def get_queryset(self):
        ...
        subproject_sectors = self.filter_list_by_delete_empty(
            self.request.GET.getlist("id_subproject_sectors[]")
        )
        subproject_types = self.filter_list_by_delete_empty(
            self.request.GET.getlist("id_subproject_types[]")
        )
        works_type_of_subprojects = self.filter_list_by_delete_empty(
            self.request.GET.getlist("id_works_type_of_subproject[]")
        )
        subproject_steps = self.filter_list_by_delete_empty(
            self.request.GET.getlist("id_subproject_step[]")
        )
        ...
        administrative_levels = AdministrativeLevel.objects.filter(
            id__in=administrative_levels_ids
        )
        ...
        if subproject_sectors:
            subprojects = subprojects.filter(
                subproject_sector__in=[elt for elt in subproject_sectors if elt]
            )

        if subproject_types:
            subprojects = subprojects.filter(
                type_of_subproject__in=[elt for elt in subproject_types if elt]
            )

        if works_type_of_subprojects:
            subprojects = subprojects.filter(
                works_type__in=[elt for elt in works_type_of_subprojects if elt]
            )
        ...
        if subproject_steps:
            ...
            for subproject in subprojects:
                ...
                if subproject_step:
                    if (
                        "not_started" in subproject_steps
                        and subproject_step == "Identifié"
                    ):
                        _subprojects.append(subproject)
                    if (
                        "in_progress" in subproject_steps
                        and subproject_step == "En cours"
                    ):
                        _subprojects.append(subproject)
                    if "completed" in subproject_steps and subproject_step in (
                        "Achevé",
                        "Réception technique",
            ...
            subprojects = Subproject.objects.filter(
                id__in=[o.id for o in _subprojects]
            ).get_actifs()
...
class DashboardSubprojectsDisplayGroupedBySectorsListView(
        ...
        for subproject in all_subprojects:
            ...
            try:
                ...
                datas[subproject.subproject_sector][_("Commune")][count] = (
                    canton.parent.name if canton else "-"
                )
            except:
                datas[subproject.subproject_sector][_("Region")][count] = "-"
                datas[subproject.subproject_sector][_("Commune")][count] = "-"
            ...
            datas[subproject.subproject_sector][_("Sites")][count] = (
                village.name
            ...
            datas[subproject.subproject_sector][_("Structures")][
                count
            ...
            # )
            obj["structures"] = datas[subproject.subproject_sector][_("Structures")][
                count
            ]

            datas[subproject.subproject_sector][_("Type")][
                count
            ] = subproject.type_of_subproject
            obj["type"] = subproject.type_of_subproject

            datas[subproject.subproject_sector][_("Companies")][count] = (
                subproject.name_of_the_awarded_company_works_companies
                if subproject.name_of_the_awarded_company_works_companies
                else "-"
            )
            obj["companies"] = datas[subproject.subproject_sector][_("Companies")][
                count
            ]

            datas[subproject.subproject_sector][_("Estimated cost") + " FCFA"][
                count
            ] = (subproject.estimated_cost if subproject.estimated_cost else 0)
            obj["estimated_cost"] = datas[subproject.subproject_sector][
                _("Estimated cost") + " FCFA"
            ][count]

            datas[subproject.subproject_sector][_("Amount FCFA TTC")][count] = (
                subproject.exact_amount_spent if subproject.exact_amount_spent else 0
            )
            obj["amount_ttc"] = datas[subproject.subproject_sector][
                _("Amount FCFA TTC")
            ][count]

            step_level = subproject.get_current_subproject_step_and_level_object

            datas[subproject.subproject_sector][_("Level of realization") + " %"][
                count
            ] = (
            ...
            obj["realization_level"] = datas[subproject.subproject_sector][
                _("Level of realization") + " %"
            ][count]

            datas[subproject.subproject_sector][_("Physical level")][
                count
            ] = subproject.get_current_subproject_step_and_level_without_percent
            obj["physical_level"] = datas[subproject.subproject_sector][
                _("Physical level")
            ][count]
    ...
    def get_context_data(self, **kwargs):
        ctx = super(
            DashboardSubprojectsDisplayGroupedBySectorsListView, self
...
class DashboardSubprojectsDisplayGroupedBySectorsListSubView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    ...
    def get_context_data(self, **kwargs):
        ctx = super(
            DashboardSubprojectsDisplayGroupedBySectorsListSubView, self
        ).get_context_data(**kwargs)
        # adls = ctx['queryset_results']['ald_filter_ids'] + ctx['queryset_results']['administrative_levels_ids']
        # all_subprojects = Subproject.objects.filter(
        #         Q(location_subproject_realized__id__in=adls) |
...
class DashboardSubprojectsDisplayGroupedByTypeListSubView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    ...
    def summary_subprojects_by_sectors_and_steps(
        ...
        for subproject in all_subprojects:
            ...
            village = subproject.get_village()
            datas[subproject.type_of_subproject][_("Sites")][count] = (
                village.name
                if village and village != "CCD"
                else (
                    f'{canton.name} ({_("Canton")})'
            ...
            datas[subproject.type_of_subproject][_("Structures")][
                count
            ] = subproject.full_title_of_approved_subproject
            # (
            #     subproject.full_title_of_approved_subproject \
            ...
            #         subproject.type_of_subproject
            # )
            obj["structures"] = datas[subproject.type_of_subproject][_("Structures")][
                count
            ]

            datas[subproject.type_of_subproject][_("Type")][
                count
            ] = subproject.type_of_subproject
            obj["type"] = datas[subproject.type_of_subproject][_("Type")][count]

            datas[subproject.type_of_subproject][_("Companies")][count] = (
                subproject.name_of_the_awarded_company_works_companies
                if subproject.name_of_the_awarded_company_works_companies
                else "-"
            )
            obj["companies"] = datas[subproject.type_of_subproject][_("Companies")][
                count
            ]

            datas[subproject.type_of_subproject][_("Estimated cost") + " FCFA"][
                count
            ] = (subproject.estimated_cost if subproject.estimated_cost else 0)
            obj["estimated_cost"] = datas[subproject.type_of_subproject][
                _("Estimated cost") + " FCFA"
            ][count]

            datas[subproject.type_of_subproject][_("Amount FCFA TTC")][count] = (
                subproject.exact_amount_spent if subproject.exact_amount_spent else 0
            )
            obj["amount_ttc"] = datas[subproject.type_of_subproject][
                _("Amount FCFA TTC")
            ][count]

            step_level = subproject.get_current_subproject_step_and_level_object

            datas[subproject.type_of_subproject][_("Level of realization") + " %"][
                count
            ] = (
                step_level.percent if step_level and step_level.percent else 0
            )  # str(int(step_level.percent)).rjust(characters_length,'0') if step_level and step_level.percent else 0
            obj["realization_level"] = datas[subproject.type_of_subproject][
                _("Level of realization") + " %"
            ][count]

            datas[subproject.type_of_subproject][_("Physical level")][
                count
            ] = subproject.get_current_subproject_step_and_level_without_percent
            obj["physical_level"] = datas[subproject.type_of_subproject][
                _("Physical level")
            ][count]
    ...
    def get_context_data(self, **kwargs):
        ctx = super(
            DashboardSubprojectsDisplayGroupedByTypeListSubView, self
        ).get_context_data(**kwargs)
