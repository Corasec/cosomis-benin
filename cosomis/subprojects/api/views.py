...
from subprojects.serializers import (
    SubprojectWithChildrenLinkedSerializer,
    SaveSubprojectSerializer,
        ...
        if administrativelevel_id:
            ...
            subprojects = subprojects.filter(
                Q(
                    ...
                    location_subproject_realized__id=administrativelevel_id,
                )
                | Q(
                    link_to_subproject=None,
                    location_subproject_realized__parent__id=administrativelevel_id,
                )
            ...
            cvd_id = int(cvd_id)
            subprojects = subprojects.filter(Q(link_to_subproject=None, cvd__id=cvd_id))

        elif project_id:
            project_id = int(project_id)
            subprojects = subprojects.filter(Q(link_to_subproject__id=project_id))
        else:
            subprojects = subprojects.filter(link_to_subproject=None)
...
class RestGetSubprojectByUser(APIView):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        try:
            return Response(
                SubprojectWithChildrenLinkedSerializer(
                    Subproject.objects.get(id=pk)
                ).data,
                status=status.HTTP_200_OK,
            )
        except Exception as exc:
            return Response({"error": exc.__str__()}, status=status.HTTP_404_NOT_FOUND)
...
class RestSaveSubproject(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                SubprojectWithChildrenLinkedSerializer(
                    Subproject.objects.get(id=request.data["pk"])
                ).data,
                status=status.HTTP_200_OK,
            )
        except Exception as exc:
