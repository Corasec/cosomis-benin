...
def get_facilitators_village_liste(
    develop_mode=False, training_mode=False, no_sql_db=False, only_ids=True
):
    administrative_levels = []
    nsc = NoSQLClient()
    if no_sql_db:
        facilitators = Facilitator.objects.using("cdd").filter(
...
def delete_administrative_levels_who_are_not_id_in_sql_db():
    ...
    for _doc in docs:
        ...
        if (
            ...
            and doc.get("administrative_id")
            and doc.get("administrative_id") != "1"
        ):
            try:
...
def save_facilitator_assignment_in_mis(
    project_id: int, develop_mode=False, training_mode=False, no_sql_db=False
):
    print(">>> Start!")
    nsc = NoSQLClient()
    if no_sql_db:
    ...
    for f in facilitators:
        ...
        if docs:
            ...
            for ad in doc.get("administrative_levels"):
                ...
                if (
                    id_str
                    and str(id_str).isdigit()
                    and not AssignAdministrativeLevelToFacilitator.objects.filter(
                        administrative_level_id=int(id_str),
    ...
    for s in liste:
        if not subproject.check_step(s):
            ...
            if subproject.approval_date_cora and s.ranking == 1:  # 1
                subproject_step.begin = (
                    subproject.approval_date_cora - datetime.timedelta(days=1)
            ...
            elif subproject.date_signature_contract_work_companies and s.ranking in (
                5,
                6,
            ):
                if s.ranking == 6:
                ...
                if subproject_step_current:
                    subproject_step.begin = (
                        subproject_step_current.begin + datetime.timedelta(days=1)
...
def save_subproject_tracking():
    ...
    for subproject in subprojects:
        if (
            ...
            or subproject.current_status_of_the_site
        ):
            current_level = strip_accents(
                subproject.current_level_of_physical_realization_of_the_work
                if subproject.current_level_of_physical_realization_of_the_work
                else ""
            ).title()
            current_status_of_the_site = strip_accents(
                subproject.current_status_of_the_site
                if subproject.current_status_of_the_site
                else ""
            ).title()
            if (
                current_level == "Remise du site".title()
                or current_status_of_the_site == "Remise du site".title()
            ):  # Remise du site
                ...
                set_step(
                    ...
                    [
                        ...
                        step_contract_signed,
                        step_site_handover,
                    ],
                )
            elif (
                "se de validation".title() in current_level
                or "se de validation".title() in current_status_of_the_site
            ):  # En phase de validation | En phse de validation
                # print("En phase de validation")
                set_step(subproject, [step_identifie])
            elif (
                "notification de l'intention d'attribution".title() in current_level
                or "notification de l'intention d'attribution".title()
                in current_status_of_the_site
            ):  # En  phase  de notification de l'intention d'attribution  | En phse de notification de l'intention d'attribution
                ...
                "Acheve".title(),
                "Acheve non receptionne".title(),
            ) or current_status_of_the_site in (
                "Acheve".title(),
                "Acheve non receptionne".title(),
            ):  # Acheve
                # print("Acheve")
                ...
                current_level == "Reception technique".title()
                or current_status_of_the_site == "Reception technique".title()
            ):  # Reception technique
                ...
                set_step(
                    ...
                    [
                        ...
                        step_progress,
                        step_completed,
                        step_technical_acceptance,
                    ],
                )
            elif (
                current_level == "Reception provisoire".title()
                or current_status_of_the_site == "Reception provisoire".title()
                ...
                set_step(
                    ...
                    [
                        ...
                        step_handover_to_the_community,
                    ],
                )
            elif (
                current_level == "Reception definitive".title()
                or current_status_of_the_site == "Reception definitive".title()
                ...
                current_level == "Infructueux".title()
                or current_status_of_the_site == "Infructueux".title()
            ):  # Infructueux
                # print("Infructueux")
                set_step(subproject, [step_identifie, step_approved, step_dao_progress])
            elif (
                current_level == "Arret travaux".title()
                or current_status_of_the_site == "Arret travaux".title()
            ):
                # print("Arrêt travaux")
                set_step(
                    subproject,
                ...
                current_level == "En cours".title()
                or current_status_of_the_site == "En cours".title()
            ):
                # print("En cours")
                set_step(
                    subproject,
                ...
                _percent = 0.0
                _current_level_of_physical_realization_of_the_works = (
                    subproject.current_level_of_physical_realization_of_the_work.split(
                        "%"
                    )
                    if subproject.current_level_of_physical_realization_of_the_work
                    else []
                ...
                if _current_level_of_physical_realization_of_the_works:
                    ...
                    if (
                        ...
                        or not str(_current_level_of_physical_realization_of_the_work)
                        .replace(".", "", 1)
                        .replace(",", "", 1)
                        .isdigit()
                    ):
