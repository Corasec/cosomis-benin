...
def compress_image(
    image: InMemoryUploadedFile, target_size_mb: float = 1
    ...
    while True:
        ...
        if (
            abs(current_size_bytes - target_size_bytes) < 1024
...
def compress_file(file_content, target_size_mb: float = 1):
    ...
    while True:
        ...
        compressed_content = gzip.compress(
            file_content, compresslevel=compression_quality
        )
        ...
        if (
            abs(current_size_bytes - target_size_bytes) < 1024
            or compression_quality < 1
            or compression_quality >= 9
        ):  # Adjust the tolerance as needed
            break
