...
class DashboardSubprojectsMixin(ModalListMixin):
    ...
    def get_queryset(self):
        administrative_level_ids_get = self.request.GET.getlist(
            "administrative_level_id[]", None
        )
        administrative_level_type = self.request.GET.get(
        ...
        administrative_levels = (
            []
        )  # AdministrativeLevel.objects.filter(id__in=administrative_levels_ids)
        if administrative_level_type == "All":
            ...
            administrative_levels = AdministrativeLevel.objects.filter(
                parent__id__in=ald_filter_ids
            )
        elif administrative_level_type:
            administrative_levels = AdministrativeLevel.objects.filter(
                parent__type=administrative_level_type
            )

        if not administrative_levels:
            administrative_levels = AdministrativeLevel.objects.filter(
                id__in=ald_filter_ids
            )
        ...
        return {
            ...
            "administrative_level_type": (
                administrative_level.type if administrative_level else ""
            ),
            "columns_tuples": list(
                administrative_levels.filter(
                    Q(type=administrative_level.type) if administrative_level else Q()
                )
                .order_by("name")
                .values_list("id", "name")
            ),
...
class DashboardSubprojectsListView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["total_without_link"] = all_subprojects.filter(
            link_to_subproject=None, subproject_type_designation="Subproject"
        ).count()
        ctx["total_subproject"] = all_subprojects.filter(
            subproject_type_designation="Subproject"
        ).count()
        ctx["total_infrastruture"] = all_subprojects.filter(
            subproject_type_designation="Infrastructure"
        ).count()
        ctx["total_latrine_blocks"] = all_subprojects.filter(
            has_latrine_blocs=True
        ).count()
        ctx["total_number_latrine_blocks"] = all_subprojects.filter(
            has_latrine_blocs=True
        ).aggregate(Sum("number_of_latrine_blocks"))["number_of_latrine_blocks__sum"]
        ctx["total_number_latrine_blocks"] = (
            ctx["total_number_latrine_blocks"]
            if ctx["total_number_latrine_blocks"]
            else 0
        )
        ctx["total_fences"] = all_subprojects.filter(has_fence=True).count()
        ctx["total_infrastrutures"] = (
            ctx["total"] + ctx["total_latrine_blocks"] + ctx["total_fences"]
        )
        ...
        ctx["total_subproject_not_started"] = ctx["total_subproject"] - (
            ctx["total_subproject_in_progress"] + ctx["total_subproject_completed"]
        )

        ctx["total_infrastruture_in_progress"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_IN_PROGRESS_STATUS
        ).count()
        ctx["total_infrastruture_completed"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_COMPLETED_STATUS
        ).count()
        ctx["total_infrastruture_not_started"] = ctx["total"] - (
            ctx["total_infrastruture_in_progress"]
            + ctx["total_infrastruture_completed"]
        )

        ctx["total_latrines_in_progress"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_IN_PROGRESS_STATUS,
            has_latrine_blocs=True,
        ).count()
        ctx["total_latrines_completed"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_COMPLETED_STATUS,
            has_latrine_blocs=True,
        ).count()
        ctx["total_latrines_not_started"] = ctx["total_latrine_blocks"] - (
            ctx["total_latrines_in_progress"] + ctx["total_latrines_completed"]
        )

        ctx["total_number_latrine_in_progress"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_IN_PROGRESS_STATUS,
            has_latrine_blocs=True,
        ).aggregate(Sum("number_of_latrine_blocks"))["number_of_latrine_blocks__sum"]
        ctx["total_number_latrine_in_progress"] = (
            ctx["total_number_latrine_in_progress"]
            if ctx["total_number_latrine_in_progress"]
            else 0
        )
        ctx["total_number_latrine_completed"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_COMPLETED_STATUS,
            has_latrine_blocs=True,
        ).aggregate(Sum("number_of_latrine_blocks"))["number_of_latrine_blocks__sum"]
        ctx["total_number_latrine_completed"] = (
            ctx["total_number_latrine_completed"]
            if ctx["total_number_latrine_completed"]
            else 0
        )
        ctx["total_number_latrine_not_started"] = ctx["total_number_latrine_blocks"] - (
            ctx["total_number_latrine_in_progress"]
            + ctx["total_number_latrine_completed"]
        )

        ctx["total_fences_in_progress"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_IN_PROGRESS_STATUS, has_fence=True
        ).count()
        ctx["total_fences_completed"] = all_subprojects.filter(
            current_status_of_the_site__in=STRUCTURE_COMPLETED_STATUS, has_fence=True
        ).count()
        ctx["total_fences_not_started"] = ctx["total_fences"] - (
            ctx["total_fences_in_progress"] + ctx["total_fences_completed"]
        )

        ctx["total_infrastrutures_in_progress"] = (
            ctx["total_infrastruture_in_progress"]
            + ctx["total_latrines_in_progress"]
            + ctx["total_fences_in_progress"]
        )
        ctx["total_infrastrutures_completed"] = (
            ctx["total_infrastruture_completed"]
            + ctx["total_latrines_completed"]
            + ctx["total_fences_completed"]
        )
        ctx["total_infrastrutures_not_started"] = (
            ctx["total_infrastruture_not_started"]
            + ctx["total_latrines_not_started"]
            + ctx["total_fences_not_started"]
        )

        ctx["number_subproject_infrastrutures"] = {
            "title": _(
                "Number of subprojects selected in relation to infrastructure by sector"
            ),
            "labels": sectors,
            "bars": [
                {
                ...
                },
                {
                    ...
                    "data": [
                        (
                            all_subprojects.filter(subproject_sector=sector).count()
                            + all_subprojects.filter(
                                subproject_sector=sector, has_latrine_blocs=True
                            ).count()
                            + all_subprojects.filter(
                                subproject_sector=sector, has_fence=True
                            ).count()
                        )
                        for sector in sectors
                    ],
        ...
        ctx["amount_subproject_infrastrutures"] = {
            "title": _(
                "Amount of subprojects selected in relation to infrastructure by sector"
            ),
            "labels": sectors,
            "bars": [
                # {
                ...
                # },
                {
                    ...
                    "data": [
                        (elt if elt else 0)
                        for elt in [
                            all_subprojects.filter(subproject_sector=sector).aggregate(
                                Sum("estimated_cost")
                            )["estimated_cost__sum"]
                            for sector in sectors
                        ]
        ...
        bars_type_structures = [
            {
                ...
                "data": [
                    (
                        (
                            all_subprojects.filter(
                                type_of_subproject__istartswith="Bâtiment Scolaire",
                                has_latrine_blocs=True,
                            ).count()
                            if type_structure == "Latrine Scolaire"
                            else (
                                all_subprojects.filter(
                                    type_of_subproject__istartswith="Pédiatrie",
                                    has_fence=True,
                                ).count()
                                if type_structure == "Clôture Pédiatrie"
                                else (
                                    all_subprojects.filter(
                                        type_of_subproject__istartswith="Bâtiment Scolaire",
                                        has_fence=True,
                        ...
                        else all_subprojects.filter(
                            type_of_subproject__istartswith=type_structure
                        ).count()
                    )
                ...
                "classrooms": [
                    ...
                    for elt in [
                        (
                            all_subprojects.filter(
                                type_of_subproject__istartswith=type_structure,
                                number_of_classrooms__isnull=False,
                            ).aggregate(Sum("number_of_classrooms"))[
                                "number_of_classrooms__sum"
                            ]
                        )
                        for type_structure in type_structures
                    ]
                ...
                "data": [
                    (
                        (
                            ...
                            .exclude(
                                current_status_of_the_site__in=(
                                    STRUCTURE_COMPLETED_STATUS
                                    + STRUCTURE_IN_PROGRESS_STATUS
                                )
                            )
        ...
        ctx["sum_bars_type_structures"] = [
            sum(elt.get("data")) for elt in bars_type_structures
        ]
                ...
                [
                    (
                        sector,
                        {
                                        ...
                                        list(
                                            set(
                                                list(
                                                    all_subprojects.filter(
                                                        subproject_sector=sector
                                                    ).values_list("type_of_subproject")
                                                )
                                            )
                                        )
                                    )
                            ...
                            "total": {
                                "subprojects": all_subprojects.filter(
                                    subproject_type_designation="Subproject",
                                    subproject_sector=sector,
                                ).count(),
                                "infrastructures": all_subprojects.filter(
                                    subproject_sector=sector
                                ).count(),
                                ...
                                [
                                    (
                                        ...
                                        all_subprojects.filter(
                                            subproject_sector=sector,
                                            type_of_subproject=t[0],
                                            current_status_of_the_site__in=STRUCTURE_NOT_START_STATUS,
                                        ).count(),
                                    )
                                    for t in sorted(
                                        list(
        ...
        ctx["administrative_level_id"] = self.request.GET.getlist(
            "administrative_level_id[]", []
        )
        administrative_level_type = self.request.GET.get(
            "administrative_level_type", "All"
        ).title()
        ctx["administrative_level_type"] = (
            "All"
            if administrative_level_type in ("", "null", "undefined")
            else administrative_level_type
        )
...
class DashboardFinancingListView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        sectors = ctx["queryset_results"]["sectors"]
        ids = (
            ctx["queryset_results"]["ald_filter_ids"].copy()
            + ctx["queryset_results"]["administrative_levels_ids"].copy()
        )
        if ids:
            ...
            allocations_project = AdministrativeLevelAllocation.objects.filter(
                project_id=1, cvd=None, administrative_level__type="Canton"
            )
        ...
        for component, component_id in {
            ...
            financing_components[component][
                "total_amount_subprojects_estimated_cost"
            ] = all_subprojects.filter(component_id=component_id).aggregate(
                Sum("estimated_cost")
            )[
                "estimated_cost__sum"
            ]
            financing_components[component][
                ...
                if financing_components[component][
                    "total_amount_subprojects_estimated_cost"
                ]
                else 0
            )
            ...
            financing_components[component][
                "total_amount_subprojects_contract_amount_work_companies"
            ] = all_subprojects.filter(component_id=component_id).aggregate(
                Sum("contract_amount_work_companies")
            )[
                "contract_amount_work_companies__sum"
            ]
            financing_components[component][
                "total_amount_subprojects_contract_amount_work_companies"
            ] = (
            ...
            financing_components[component]["total_allocations_cantons"] = (
                allocations_project.filter(
                    cvd=None, component_id=component_id
                ).aggregate(Sum("amount"))["amount__sum"]
            )
            financing_components[component]["total_allocations_cantons"] = (
                financing_components[component]["total_allocations_cantons"]
                if financing_components[component]["total_allocations_cantons"]
                else 0
            )

            financing_components[component][
                "total_amount_remaining_after_allocation"
            ] = (
                financing_components[component]["total_allocations_cantons"]
                - financing_components[component][
                    "total_amount_subprojects_estimated_cost"
            ...
            financing_components[component]["total_amount_residual"] = (
                financing_components[component]["total_allocations_cantons"]
                - financing_components[component][
                    "total_amount_subprojects_contract_amount_work_companies"
                ]
        ...
        ctx["total_amount_subprojects_estimated_cost"] = all_subprojects.aggregate(
            Sum("estimated_cost")
        )["estimated_cost__sum"]
        ctx["total_amount_subprojects_estimated_cost"] = (
            ctx["total_amount_subprojects_estimated_cost"]
            if ctx["total_amount_subprojects_estimated_cost"]
            else 0
        )

        ctx["total_amount_subprojects_contract_amount_work_companies"] = (
            all_subprojects.aggregate(Sum("contract_amount_work_companies"))[
                "contract_amount_work_companies__sum"
            ]
        )
        ctx["total_amount_subprojects_contract_amount_work_companies"] = (
            ctx["total_amount_subprojects_contract_amount_work_companies"]
            if ctx["total_amount_subprojects_contract_amount_work_companies"]
            else 0
        )

        ctx["total_allocations_cantons"] = allocations_project.filter(
            cvd=None
        ).aggregate(Sum("amount"))["amount__sum"]
        ctx["total_allocations_cantons"] = (
            ctx["total_allocations_cantons"] if ctx["total_allocations_cantons"] else 0
        )

        ctx["total_amount_remaining_after_allocation"] = (
            ctx["total_allocations_cantons"]
            - ctx["total_amount_subprojects_estimated_cost"]
        )
        ctx["total_amount_residual"] = (
            ctx["total_allocations_cantons"]
            - ctx["total_amount_subprojects_contract_amount_work_companies"]
        )
...
class DashboardFinancingListByCantonView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        if ids:
            lines = AdministrativeLevelWave.objects.filter(
                administrative_level__id__in=ids
            ).order_by("administrative_level__name")
            ...
            lines = AdministrativeLevelWave.objects.all().order_by(
        ...
        admls_children = [
            (
                ...
                ([adl.id] + get_administrative_level_ids_descendants(adl.id, None, [])),
            )
            for adl in admls
        ]
        ...
        ctx["amount_cantons_component_1_1"] = {
            "title": _(
                "Presentation of allocations, estimates and expenditure by canton - Component 1.1"
            ),
            ...
            "bars": [
                {
                    ...
                    "data": [
                        ...
                        for elt in [
                            adml[1]
                            .filter(component_id=2)
                            .aggregate(Sum("amount"))["amount__sum"]
                            for adml in admls_children
                        ]
                    ],
                },
                {
                    ...
                    "data": [
                        ...
                        for elt in [
                            all_subprojects.filter(
                                ...
                                | Q(canton__id__in=adml[2]),
                                component_id=2,
                            ).aggregate(Sum("contract_amount_work_companies"))[
                                "contract_amount_work_companies__sum"
                            ]
                            for adml in admls_children
                        ]
        ...
        ctx["amount_cantons_component_1_2"] = {
            "title": _(
                "Presentation of allocations, estimates and expenditure by canton - Component 1.2"
            ),
            "labels": [adl.name for adl in admls],
            "bars": [
                {
        ...
        ctx["amount_cantons_component_1_3"] = {
            "title": _(
                "Presentation of allocations, estimates and expenditure by canton - Component 1.3"
            ),
            "labels": [adl.name for adl in admls],
            "bars": [
                {
...
class SubprojectsDetailsModalView(
    DashboardSubprojectsMixin,
    AJAXRequestMixin,
    LoginRequiredMixin,
    JSONResponseMixin,
    generic.ListView,
    ...
    def get_context_data(self, **kwargs):
        ...
        if "subprojects-" in list_type_search:
            ...
            list_name_search = _(
                "Structures/infrastructures without latrines and fences"
            )
        elif list_type_search == "infrastruture-completed":
            list_name_search = _(
                "Structures/infrastructures completed (excluding latrines and fences)"
            )
            all_subprojects = all_subprojects.filter(
                current_status_of_the_site__in=STRUCTURE_COMPLETED_STATUS
            )
            ...
            list_name_search = _(
                "Structures/infrastructures in progress (excluding latrines and fences)"
            )
            all_subprojects = all_subprojects.filter(
                current_status_of_the_site__in=STRUCTURE_IN_PROGRESS_STATUS
            )
            ...
            list_name_search = _(
                "Structures/infrastructures not start (excluding latrines and fences)"
            )
            all_subprojects = all_subprojects.exclude(
                current_status_of_the_site__in=(
                    STRUCTURE_COMPLETED_STATUS + STRUCTURE_IN_PROGRESS_STATUS
                )
            )
            ...
            if list_type_search == "infrastrutures-with-latrines-fences":
                list_name_search = _(
                    "Structures/infrastructures with latrines and fences"
                )
            elif list_type_search == "infrastrutures-with-latrines-fences-completed":
                list_name_search = _(
                    "Structures/infrastructures with latrines and fences completed"
                )
                all_subprojects = all_subprojects.filter(
                    current_status_of_the_site__in=STRUCTURE_COMPLETED_STATUS
                )
                ...
                list_name_search = _(
                    "Structures/infrastructures with latrines and fences in progress"
                )
                all_subprojects = all_subprojects.filter(
                    current_status_of_the_site__in=STRUCTURE_IN_PROGRESS_STATUS
                )
                ...
                list_name_search = _(
                    "Structures/infrastructures with latrines and fences not start"
                )
                all_subprojects = all_subprojects.exclude(
                    current_status_of_the_site__in=(
                        STRUCTURE_COMPLETED_STATUS + STRUCTURE_IN_PROGRESS_STATUS
            ...
            context["total_latrine_blocks"] = all_subprojects.filter(
                has_latrine_blocs=True
            ).count()
            context["total_fences"] = all_subprojects.filter(has_fence=True).count()
            context["total_infrastrutures"] = (
                context["total"]
                + context["total_latrine_blocks"]
                + context["total_fences"]
            )
            list_name_search += f"\
        ...
        context["administrative_level_id"] = self.request.GET.getlist(
            "administrative_level_id[]", []
        )
        context["administrative_level_type"] = (
            "All"
            if administrative_level_type in ("", "null", "undefined")
            else administrative_level_type
        )
