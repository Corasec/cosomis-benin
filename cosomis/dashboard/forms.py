...
class AdministrativeLevelFilterForm(forms.Form):
    ...
    def __init__(self, has_all=True, *args, **kwargs):
        ...
        query_result_regions = init_list + list(
            administrativelevels_docs.filter(type="Region").values_list("id", "name")
        )
        query_result_prefectures = init_list + list(
            administrativelevels_docs.filter(type="Prefecture").values_list(
                "id", "name"
            )
        )
        query_result_communes = init_list + list(
            administrativelevels_docs.filter(type="Commune").values_list("id", "name")
        )
        query_result_cantons = init_list + list(
            administrativelevels_docs.filter(type="Canton").values_list("id", "name")
        )
        query_result_villages = init_list + list(
            administrativelevels_docs.filter(type="Village").values_list("id", "name")
        )
