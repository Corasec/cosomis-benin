...
class DashboardSubprojectsListView(
    ...
    def summary_subprojects_by_sectors_1(
        self, columns_listes, sectors, all_subprojects, characters_length
    ):
        datas = {_("Sectors"): {}, _("Type"): {}}
        ...
        for i in range(len(_sectors)):
            ...
            for column in columns_listes:
                ...
                for _subp in all_subprojects:
                    if (
                        (
                            _subp.location_subproject_realized_id in column[2]
                            or _subp.canton_id in column[2]
                        )
                        and _subp.subproject_sector == _sectors[i]
                        and _subp.subproject_type_designation
                        in (
            ...
            datas[_("Total")][
                count
            ] = number_toal  # str(number_toal).rjust(characters_length,'0')
            count += 1
        ...
        for i in range(3):
            for k_data in datas.keys():
                ...
                if k_data not in columns_skip:
                    _sum = functions.sum_dict_value(
                        datas[k_data], count, pair=bool(i) if i < 2 else True
                    )
                if _sum:
                    datas[k_data][
                        count + i
                    ] = _sum  # str(_sum).rjust(characters_length,'0')
        # End All sum
    ...
    def get_context_data(self, **kwargs):
        ...
        for column in columns_tuples:
            ids_descendants = get_administrative_level_ids_descendants(
                column[0], None, []
            )
            columns_listes.append(
                [
                    ...
                    column[1],
                    (
                        [column[0]] + ids_descendants
                        if column[0] != "All"
                        else ids_descendants
                    ),
                ]
            )
        ...
        for k, v in self.summary_subprojects_by_sectors_1(
            columns_listes, sectors, all_subprojects, characters_length
        ).items():
...
class DashboardSubprojectsBySectorAmountListView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    template_name = "tracking.html"
    context_object_name = "queryset_results"

    def summary_amount_subprojects_by_sectors_1(
        self, columns_listes, sectors, all_subprojects, characters_length
    ):
        datas = {
            _("Sectors"): {},
        ...
        for i in range(len(_sectors)):
            ...
            subprojects_filter_infras = all_subprojects.filter(
                subproject_sector=_sectors[i],
                subproject_type_designation__in=["Infrastructure", "Subproject"],
            )
            datas[_("Number of subproject infrastructures")][
                count
            ] = subprojects_filter_infras.count()
            ...
            for column in columns_listes:
                ...
                for _subp_structure in subprojects_filter_infras:
                    if (
                        _subp_structure.location_subproject_realized_id in column[2]
                        or _subp_structure.canton_id in column[2]
                    ):
                        estimated_cost += _subp_structure.estimated_cost

                datas[column[1]][
                    count
                ] = estimated_cost  # str(estimated_cost).rjust(characters_length,'0')
                toal_mount += estimated_cost

            datas[_("Total")][
                count
            ] = toal_mount  # str(toal_mount).rjust(characters_length,'0')
            count += 1
    ...
    def get_context_data(self, **kwargs):
        ctx = super(DashboardSubprojectsBySectorAmountListView, self).get_context_data(
            **kwargs
        )
        # subprojects = ctx['queryset_results']['subprojects']
        sectors = ctx["queryset_results"]["sectors"]
        ...
        for k, v in self.summary_amount_subprojects_by_sectors_1(
            columns_listes, sectors, all_subprojects, characters_length
        ).items():
...
class DashboardSubprojectsSectorsAndStepsListView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    template_name = "tracking.html"
    context_object_name = "queryset_results"
    ...
    def summary_subprojects_by_sectors_and_steps(
        self, all_subprojects, characters_length
    ):
        datas = {
            _("N°"): {},
            _("Region"): {},
        ...
        for subproject in all_subprojects:
            ...
            try:
                ...
                datas[_("Region")][count] = (
                    canton.parent.parent.parent.name if canton else "-"
                )
            except:
                datas[_("Region")][count] = "-"
            ...
            datas[_("Sites")][count] = (
                village.name
                if village and village != "CCD"
                else (
                    f'{canton.name} ({_("Canton")})'
                    if village == "CCD" and canton
                    else "-"
                )
            )
            ...
            datas[_("Structures")][count] = (
                ...
                if subproject.subproject_type_designation != "Infrastructure"
                else subproject.type_of_subproject
            )
            datas[_("Companies")][count] = (
                subproject.name_of_the_awarded_company_works_companies
                if subproject.name_of_the_awarded_company_works_companies
                else "-"
            )
            datas[_("Estimated cost") + " FCFA"][count] = (
                subproject.estimated_cost if subproject.estimated_cost else 0
            )
            datas[_("Amount FCFA TTC")][count] = (
                subproject.contract_amount_work_companies
                if subproject.contract_amount_work_companies
                else 0
            )

            step_level = subproject.get_current_subproject_step_and_level_object

            datas[_("Level of realization") + " %"][count] = (
                step_level.percent if step_level and step_level.percent else 0
            )  # str(int(step_level.percent)).rjust(characters_length,'0') if step_level and step_level.percent else 0
            datas[_("Physical level")][
                count
            ] = subproject.get_current_subproject_step_and_level_without_percent
        ...
        return {
            "title": (
                table_title
                if table_title
                else _("Subprojects by village, sector and step")
            ),
            "datas": datas,
            "length_loop": range(0, count + 1),
            "values": list(datas.values()),
    ...
    def get_context_data(self, **kwargs):
        ctx = super(DashboardSubprojectsSectorsAndStepsListView, self).get_context_data(
            **kwargs
        )
        adls = (
            ctx["queryset_results"]["ald_filter_ids"]
            + ctx["queryset_results"]["administrative_levels_ids"]
        )
        all_subprojects = Subproject.objects.filter(
            Q(location_subproject_realized__id__in=adls) | Q(canton__id__in=adls)
        ).get_actifs()
        ...
        for k, v in self.summary_subprojects_by_sectors_and_steps(
            all_subprojects, characters_length
        ).items():
...
class DashboardSubprojectsStepsAlreadyTrackListView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    template_name = "tracking.html"
    context_object_name = "queryset_results"
    ...
    def summary_subprojects_by_steps_already_track(
        self, all_subprojects, characters_length, administrative_levels_ids
    ):
        datas = {_("Step"): {}, _("Number"): {}}
        ...
        for sub in all_subprojects:
            if sub.current_status_of_the_site == "En cours":
                datas[_("Number")][1] = (
                    1
                    if datas[_("Number")].get(1) == None
                    else datas[_("Number")].get(1) + 1
                )

            elif sub.current_status_of_the_site in (
                "Achevé",
                "Réception technique",
                ...
                datas[_("Number")][2] = (
                    1
                    if datas[_("Number")].get(2) == None
                    else datas[_("Number")].get(2) + 1
                )
                if sub.current_status_of_the_site == "Réception technique":
                    datas[_("Number")][3] = (
                        1
                        if datas[_("Number")].get(3) == None
                        else datas[_("Number")].get(3) + 1
                    )
                elif sub.current_status_of_the_site == "Réception provisoire":
                    datas[_("Number")][4] = (
                        1
                        if datas[_("Number")].get(4) == None
                        else datas[_("Number")].get(4) + 1
                    )
                elif sub.current_status_of_the_site == "Réception définitive":
                    datas[_("Number")][5] = (
                        1
                        if datas[_("Number")].get(5) == None
                        else datas[_("Number")].get(5) + 1
                    )

            else:
                datas[_("Number")][0] = (
                    1
                    if datas[_("Number")].get(0) == None
                    else datas[_("Number")].get(0) + 1
                )
    ...
    def get_context_data(self, **kwargs):
        ctx = super(
            DashboardSubprojectsStepsAlreadyTrackListView, self
        ...
        for k, v in self.summary_subprojects_by_steps_already_track(
            all_subprojects,
            characters_length,
            ctx["queryset_results"]["administrative_levels_ids"],
        ).items():
...
class DashboardSubprojectsCurrentStepsListView(
    DashboardSubprojectsMixin, AJAXRequestMixin, LoginRequiredMixin, generic.ListView
):
    template_name = "tracking.html"
    context_object_name = "queryset_results"
    ...
    def summary_subprojects_by_current_steps(
        self, all_subprojects, characters_length, administrative_levels_ids
    ):
        datas = {_("Step"): {}, _("Number"): {}}
        ...
        administrative_levels_ids_str = ", ".join(
            str(elt) for elt in administrative_levels_ids
        )
        count = 0
        with connection.cursor() as cursor:
            try:
...
"""
    ...
    def get_context_data(self, **kwargs):
        ctx = super(DashboardSubprojectsCurrentStepsListView, self).get_context_data(
            **kwargs
        )
        all_subprojects = ctx["queryset_results"]["subprojects"]
        ...
        for k, v in self.summary_subprojects_by_current_steps(
            all_subprojects,
            characters_length,
            ctx["queryset_results"]["administrative_levels_ids"],
        ).items():
