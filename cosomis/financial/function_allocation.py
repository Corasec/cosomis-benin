        ...
        while count < long:
            ...
            try:
                ...
                if _canton:
                    ...
                    allocation_1_1_v1 = get_value(
                        datas_file["Allocation sc 1.1"][count]
                    )
                    allocation_1_1_v2 = get_value(
                        datas_file["Allocation sc 1.1 V2"][count]
                    )
                    allocation_1_1_cfa = get_value(
                        datas_file[
                            """FCFA
1.1"""
                        ][count]
...
1.3"""
                    ...
                    try:
                        administrative_level_canton = AdministrativeLevel.objects.get(
                            name=canton_file_data, type="Canton"
                        )
                        ...
                        text_errors += (
                            ...
                            if text_errors
                            else f"Line N°{count} [{canton_file_data}]: {exc.__str__()}"
                        )
                    ...
                    if not _is_object_error and administrative_level_canton:
                        for k_id, v_money in components.items():
                            ...
                            allocation.administrative_level = (
    ...
    if at_least_one_save and not at_least_one_error:
        ...
        if at_least_error_name:
            message = _(
                "A problem has occurred! The name(s) of the canton(s) is wrong."
            )
        else:
        ...
        if at_least_error_name:
            message = _(
                "Some element(s) have not been saved! The name(s) of the canton(s) is wrong."
            )
        else:
    ...
    file_path = (
        "logs/errors/upload_allocations_logs_errors_"
...
def sum_allocation_amount_by_component(component_id, is_administrative_level=True):
    return AdministrativeLevelAllocation.objects.filter(
        (
            Q(administrative_level__isnull=False)
            if is_administrative_level
...
def sum_allocation_amount_in_dollars_by_component(
    component_id, is_administrative_level=True
):
    return AdministrativeLevelAllocation.objects.filter(
        (
            Q(administrative_level__isnull=False)
...
def sum_allocation_amount_by_administrative_level(
    administrative_level_id, is_administrative_level=True
):
    return AdministrativeLevelAllocation.objects.filter(
        Q(administrative_level_id=administrative_level_id)
        if is_administrative_level
        else Q(cvd_id=administrative_level_id)
    ).aggregate(Sum("amount"))["amount__sum"]


def sum_allocation_amount_in_dollars_by_administrative_level(
    administrative_level_id, is_administrative_level=True
):
    return AdministrativeLevelAllocation.objects.filter(
        Q(administrative_level_id=administrative_level_id)
