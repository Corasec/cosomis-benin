...
class AdministrativeLevelAllocationForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["administrative_level"].choices = [("", "---------")] + [
            (o.id, f'{o.name} ({o.type} {_("Of")} {o.parent if o.parent else "TOGO" })')
            for o in AdministrativeLevel.objects.filter().order_by("name")
        ]

        self.fields["cvd"].choices = [("", "---------")] + [
            (o.id, f"{o.name} ({o.headquarters_village.parent.name})")
            for o in CVD.objects.filter().order_by("name")
        ]
    ...
    def clean(self):
        ...
        if cvd and administrative_level:
            raise forms.ValidationError(
                _("You must select either an administrative level or a CVD")
            )
        elif not cvd and not administrative_level:
            raise forms.ValidationError(
                _("You must choose an administrative level or a CVD")
            )
        return super().clean()
