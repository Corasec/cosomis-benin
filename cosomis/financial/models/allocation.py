...
class AdministrativeLevelAllocation(BaseModel):
    ...
    amount_in_dollars = CustomerFloatRangeField(
        verbose_name=_("Amount allocated in dollars"), min_value=0
    )
    allocation_date = models.DateField(
        verbose_name=_("Allocation Date"), null=True, blank=True
    )
    amount_after_signature_of_contracts = CustomerFloatRangeField(
        verbose_name=_(
            "Amount allocated after signature of contract(s) (This is only necessary if it's a CVD)"
        ),
        min_value=0,
        null=True,
        blank=True,
    )
    amount_in_dollars_after_signature_of_contracts = CustomerFloatRangeField(
        verbose_name=_(
            "Amount allocated in dollars after signature of contract(s) (This is only necessary if it's a CVD)"
        ),
        min_value=0,
        null=True,
        blank=True,
    )
    allocation_date_after_signature_of_contracts = models.DateField(
        verbose_name=_(
            "Allocation Date after signature of contract(s) (This is only necessary if it's a CVD)"
        ),
        null=True,
        blank=True,
    )
    ...
    def sum_amount_by_administrative_level(self):
        return AdministrativeLevelAllocation.objects.filter(
            Q(cvd_id=self.cvd.id)
            if self.cvd
            else Q(administrative_level_id=self.administrative_level.id)
